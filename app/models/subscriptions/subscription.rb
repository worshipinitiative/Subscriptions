module Subscriptions
  class Subscription < ActiveRecord::Base
    ...
    def self.new_from_template( t, subscription_params = {} )
      ...
      subscription.seat_count = t.seat_count

      return subscription
    end

    def self.cycle_subscriptions( async = true )
      Subscription.where( "next_bill_date < ?", Time.now ).each do |s|
        if async
    ...
    def uniqueness_of_ownerable
      errors.add(:ownerable, "Must be unique") if Subscription.where(ownerable_type: ownerable_type, ownerable_id: ownerable_id).count > 0
    end
